...
1. 처음에 const a = func(1); 이런식으로 예시를 놨는데 자꾸 숫자 리터럴이 타입으로 잡히길래 ???? 하고 있었는데 const로 선언하면 당연히 리터럴로 잡히지 바보야
2. 화살표 함수로 구현하는데 자꾸 `<T>`이 부분에서 에러가 나길래 찾아보니까 JSX 충돌문제를 일으킨다는걸 찾음
### Quiz 2.
...
1. rest문법은 처음에 못쓴다고 (...rest, parameter) 알고 있는데 찾아보니 사용 가능함(튜플에서는)
2. 배열 리턴 타입을 T라고 하면 unknown이랑 충돌이 나서 타입 단언으로 했는데 맞는지는 몰루...?
 ...
 * 1. 첫 번째 인수로 받은 arr은 배열이니까 (map이 배열 메서드니까) => T[]
 * 2. 두 번째 인수에 callback에서 item은 arr 배열을 순회하는 거니까 => T
 * 3. 콜백에 반환값은 무조건 T가 참이 아닐 수 있으니 U